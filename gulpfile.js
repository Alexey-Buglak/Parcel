...
const updatedImgs = () => gulp.src(path.src.img, {allowEmpty: true}).pipe(imagemin()).pipe(gulp.dest(path.build.img))
...
const updatedJs = () => gulp.src(path.src.js).pipe(concat('index.js')).pipe(uglify()).pipe(gulp.dest(path.build.js)).pipe(browserSync.stream())
